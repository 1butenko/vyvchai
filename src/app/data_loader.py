...
if __name__ == '__main__':
    ...
    # Dummy data
    pd.DataFrame({
        "page_text": ["Intro to AI."], "book_page_number": [10], "topic_title": ["AI"], "global_discipline_id": ["CS101"]
    }).to_parquet(os.path.join(DATA_PATH, "pages_for_hackathon (gemini).parquet"))
    
    pd.DataFrame({
        "student_id": [123, 123], "score_numeric": [5, 8], "topic_name": ["Алгебра: Рівняння", "Алгебра: Функції"]
    }).to_parquet(os.path.join(DATA_PATH, "benchmark_scores.parquet"))

    pd.DataFrame({
        "student_id": [123], "absence_reason": ["Sick"], "topic_name": ["Алгебра: Рівняння"]
